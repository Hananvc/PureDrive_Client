...
function VehicleEditModal({ vehicleId, onClose, fetchData }) {
  ...
  async function fetchVehicleData() {
    try {
      ...
      const response = await axiosInstance.get(`/vehicleapp/vehicle/${vehicleId}/`);
      console.log(response.data,"response in modal");
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (
      ...
      // No changes made, show a toast message
    ...
    try {
      ...
      const response = await axiosInstance.patch(`/adminapp/vehicle/${vehicleId}/`, formData);
      toast.success('Vehicle updated successfully');
      onClose();
      fetchData();
    } catch (error) {
      console.error('Error updating vehicle:', error);
      toast.error('Error in vehicle updation');
    }
  };
      ...
      <div className="relative max-w-3xl w-full p-6 overflow-x-hidden overflow-y-auto md:inset-0">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <button
            ...
            data-modal-hide="vehicle-edit-modal"
          >
            {/* Close button icon */}
          </button>
